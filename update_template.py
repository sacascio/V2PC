...
def update_template(template,ip,token,payload):
        ...
        if response.status_code == 200:
                print "Publishing template %s updated successfully!" % template
		...
		if os.path.isfile(file_name + '.json'):
			with open(file_name + '.json') as data_file:    
				...
				if kms_template != 'none':
					...
					data['properties']["isIframeEncrypt"] = "true"
				else:
					data['properties']["isIframeEncrypt"] = "false"
				
				update_template(template_name,ip,token,data)
		else:
			print "JSON File " + file_name + '.json DOES NOT EXIST'
